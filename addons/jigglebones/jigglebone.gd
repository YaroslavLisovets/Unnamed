...
func _process(delta):
	...
	var bone_rotate_axis = bone_forward_local.normalized().cross(diff_vec_local)
	...
	var bone_new_transf_obj = Transform(bone_transf_obj.basis.rotated(bone_rotate_axis_obj, bone_rotate_angle), bone_transf_obj.origin).orthonormalized() 

	if is_nan(bone_new_transf_obj[0][0]):
		bone_new_transf_obj = Transform()  # Corrupted somehow
	skeleton.set_bone_global_pose_override(bone_id, bone_new_transf_obj.orthonormalized(), 1)
